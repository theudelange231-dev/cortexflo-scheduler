...
if driver_file and shift_file:
    # Load driver data and strip column spaces
    drivers_df = pd.read_csv(driver_file)
    drivers_df.columns = drivers_df.columns.str.strip()
    drivers_df['Assigned Hours'] = 0

    # Load shift data and strip column spaces
    shifts_df = pd.read_csv(shift_file)
    ...
    for idx, row in shifts_df.iterrows():
        ...
        # Pick driver with minimum assigned hours
        if not available_drivers.empty:
            min_hours = available_drivers['Assigned Hours'].min()
            candidate_drivers = available_drivers[available_drivers['Assigned Hours'] == min_hours]
            chosen_driver = random.choice(candidate_drivers['Driver'].tolist())
